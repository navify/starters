...
import { copy, remove, writeFile } from '@navify/utils-fs';
import { readPackageJsonFile } from '@navify/cli-framework/utils/node';
...
export const NAVIFY_TYPE_DIRECTORIES = [/*'navify1',*/ 'react', 'kdu'];
...
export async function gatherChangedBaseFiles(): Promise<string[]> {
  ...
  for (const navifyType of NAVIFY_TYPE_DIRECTORIES) {
    const baseDir = path.resolve(REPO_DIRECTORY, navifyType, 'base');
...
export async function getStarterDirectories(
  ...
  const officialStarterDirs = await getDirectories(path.resolve(REPO_DIRECTORY, navifyType, STARTER_TYPE_OFFICIAL));

  if (community) {
    const communityScopes = await getDirectories(path.resolve(REPO_DIRECTORY, navifyType, STARTER_TYPE_COMMUNITY));
...
export async function buildStarters({
  ...
  for (const navifyType of NAVIFY_TYPE_DIRECTORIES) {
    const baseDir = path.resolve(REPO_DIRECTORY, navifyType, 'base');
    const starterDirs = await getStarterDirectories(navifyType);
    ...
    for (const [ref, starterDirsAtRef] of refmap.entries()) {
      if (!current) {
        console.log(`Checking out ${bold(cyan(navifyType))} base files at ${bold(ref)}`);
      ...
      await Promise.all(
        starterDirsAtRef.map(async (starterDir) => {
          ...
          const id = buildStarterId(navifyType, starterType, starterDir);
          await buildStarter(navifyType, starterType, starterDir);

          const name = rest.join('/');
          const sha1 = current ? currentSha1 : (await runcmd('git', ['rev-parse', ref])).trim();
          starterList.starters.push({ name, id, type: navifyType, ref, sha1 });
        })
      );
...
export function buildStarterId(navifyType: string, starterType: string, starterDir: string): string {
  const starter = generateStarterName(starterType, starterDir);
  const id = `${navifyType}-${starterType}-${starter}`;

  return id;
}

export async function buildStarter(navifyType: string, starterType: string, starterDir: string): Promise<void> {
  const id = buildStarterId(navifyType, starterType, starterDir);
  const baseDir = path.resolve(REPO_DIRECTORY, navifyType, 'base');
