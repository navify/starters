...
import { filter } from '@navify/utils-array';
import { readFile, readdir, stat } from '@navify/utils-fs';

import { StarterManifest, TsconfigJson } from '../definitions';

export const NAVIFY_MANIFEST_FILE = 'navify.starter.json';
...
export async function readStarterManifest(dir: string): Promise<StarterManifest | undefined> {
  try {
    return JSON.parse(await readFile(path.resolve(dir, NAVIFY_MANIFEST_FILE), { encoding: 'utf8' }));
  } catch (e) {
    // ignore
  }
}
