...
import { readFile, writeFile } from '@navify/utils-fs';

import { NAVIFY_MANIFEST_FILE, getCommandHeader, getDirectories, log } from '../utils';
import { BUILD_DIRECTORY, NAVIFY_TYPE_DIRECTORIES, REPO_DIRECTORY } from '../lib/build';
...
export class GenerateChecksumCommand extends Command {
  ...
  async run(inputs: CommandLineInputs, options: CommandLineOptions) {
    ...
    for (const type of NAVIFY_TYPE_DIRECTORIES) {
      ...
      for (const dir of builtStarters) {
        ...
        try {
          ...
          const manifest = await readFile(path.resolve(dir, NAVIFY_MANIFEST_FILE), { encoding: 'utf8' });
